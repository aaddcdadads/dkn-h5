		...
		<view class="table_box_big" :style='{height:tableHeight}'>
			...
			<view class="table_box">
				<view class="div-table div-table-head">
					<view class="thead">
						<view class="tr">
							<view class="td selection" v-if="selection=='mulit'"
							...
							<view class="td" :style='{width:countHeadColspanWidth(item,index),height:thTdHeight+"px"}'
								v-for="(item,index) in columns" :key="item.key">
						...
						<view class="div-table div-table-body">
							<checkbox-group @change="checkboxChange">
								<template v-for="(item,index) in data" :key="item.id">
									<view :class="rowClass(item, index)" @click="selectRow(item,index)">
										<view class="td selection" v-if="selection=='mulit'"
											:style='{width:selectionTdWidth,height:tdHeight+"px"}'>
											<view :class="['td_wrap']" :style='{width:selectionTdWidth,height:tdHeight+"px"}'>
												<checkbox :value="checkBoxList[index].id" color="#999" :disabled="checkBoxList[index].$disabled"
													:checked="checkBoxList[index].$checked" style="transform:scale(0.7)" />
											</view>
										</view>
										<template v-for="(tdItem,tdItemIndex) in columns" :key="tdItemIndex">
											<view class="td" :class='[
											...
											spanMethod(item,tdItem,index,tdItemIndex)["colspan"]>1?"colspan":""]' :style='{height:countRowspanHeight(item,tdItem,index,tdItemIndex),
											width:countColspanWidth(item,tdItem,index,tdItemIndex)}'>
												...
												width: countColspanWidth(item,tdItem,index,tdItemIndex,true)}'>
													...
													<template v-if="tdItem.$operateList">
														<template v-for="(btn, index) in tdItem.$operateList" :key="index">
															<button :class="[btn.styles?btn.styles:'']"
																v-bind:style="{ padding: '2px 5px',fontSize:'12px',lineHeight:'1.2',display:'inline-block'}"
																@click="pullEvent(btn.event,{row:item,index:index})" type="primary"
																size="min">{{btn.label}}</button>
														</template>
													</template>
				...
				<template v-else>
					<view class="table_tbody_box empty-data-body-box div-table-body"
						...
						<view class="tr " :style='{height:emptyColHeight,width:emptyColWidth}'>
							<view class="td" :style='{height:emptyColHeight,width:emptyColWidth}'>
								<view :class="['td_wrap']"
									:style='{height:parseInt(emptyColHeight-2)+"px",width:emptyColWidth,lineHeight:parseInt(emptyColHeight-2)+"px"}'>
				...
				<template v-if="data.length">
					<view class="fixed-left" v-if="columnsFixedLeft[0]||(selection=='mulit'&&fixedCheckbox)">
						<view class="tr fixed-thead-tr">
							<view class="td selection" v-if="selection=='mulit'&&fixedCheckbox"
								:style='{width:selectionTdWidth,height:thTdHeight+"px"}'>
								<view :class="['td_wrap']" :style='{width:selectionTdWidth,height:thTdHeight+"px"}'>
									<checkbox-group @change="checkboxChangeAll">
										<checkbox value="all" color="#999" :checked="switchAllCheckBox" style="transform:scale(0.7)" />
							...
							<view class="td" v-if="columnsFixedLeft[0]"
						...
						<view v-for="(item,index) in data" :key="item.id" :class="rowClass1(item, index)" @click="selectRow(item,index)">
							<view class="td selection fixed-td" v-if="selection=='mulit'&&fixedCheckbox"
								:style='{width:selectionTdWidth,height:tdHeight+"px"}'>
								<view :class="['td_wrap']" :style='{width:selectionTdWidth,height:tdHeight+"px"}'>
									<checkbox :value="checkBoxList[index].id" color="#999" :disabled="checkBoxList[index].$disabled"
										:checked="checkBoxList[index].$checked" style="transform:scale(0.7)" />
							...
							<view class="td fixed-td" v-if="columnsFixedLeft[0]"
								...
								<view class="td_wrap fixed-wrap"
									...
									<template v-if="columnsFixedLeft[0].$operateList">
										<template v-for="(btn, index) in columnsFixedLeft[0].$operateList" :key="index">
											<button :class="[btn.styles?btn.styles:'']"
												v-bind:style="{ padding: '2px 5px',fontSize:'12px',lineHeight:'1.2',display:'inline-block'}"
												@click="pullEvent(btn.event,{row:item,index:index})" type="primary"
												size="min">{{btn.label}}</button>
							...
							<view class="td fixed-td"
								...
								<view class="td_wrap fixed-wrap"
									...
									<template v-if="columnsFixedRight[0]&&columnsFixedRight[0].$operateList">
										<template v-for="(btn, index) in columnsFixedRight[0].$operateList" :key="index">
											<button :class="[btn.styles?btn.styles:'']"
												v-bind:style="{ padding: '2px 5px',fontSize:'12px',lineHeight:'1.2',display:'inline-block'}"
												@click="pullEvent(btn.event,{row:item,index:index})" type="primary"
												size="min">{{btn.label}}</button>
...
export default {
	...
	methods: {
		rowClass(item, index) {
			return ["tr",this.rowClassNamePlus(item,index),
									this.selection=="single"&&this.checkBoxList[index].$checked?"selected":"",
									this.selection=="single"&&this.checkBoxList[index].$disabled?"disabled":""];
		},
		rowClass1(item, index) {
			return ["tr",
						this.selection=="single"&&this.checkBoxList[index].$checked?"selected":"",
						this.selection=="single"&&this.checkBoxList[index].$disabled?"disabled":""]
		},
		//获取数据副本，轻拷贝
		asyncCheckBoxList() {
			this.checkBoxList = this.data.map(item => {
				return {
					...item
				};
			});
