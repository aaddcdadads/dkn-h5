...
export default {
  ...
  methods: {
    imageLoad(ev) {
      ...
      this.frame.width = this.croppedWidth;
      this.frame.height = this.croppedHeight;
      if (!this.fixed) {
    ...
    onok() {
      ...
      var th = height;
      if (this.fixed) {
        tw = this.croppedWidth / 2;
        th = this.croppedHeight / 2;
      } else {
        if (tw > this.croppedmaxWidth / 2) {
          var sc = this.croppedmaxWidth / 2 / tw;
          tw = tw * sc;
          th = th * sc;
        }
        if (th > this.croppedmaxHeight / 2) {
          var sc = this.croppedmaxHeight / 2 / th;
          th = th * sc;
          tw = tw * sc;
        }
      }
