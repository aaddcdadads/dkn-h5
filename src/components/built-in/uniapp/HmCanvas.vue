<template>
	<view class="demo">
		<canvas type="2d" :style="{ width: canvasInfo.width + 'px', height: canvasInfo.height + 'px' }" id="myCanvas" />
	</view>
</template>
<script>
export default {
	...
	async mounted() {
		...
		this.draw(canvas, ctx, width, height)
	},

	methods: {
		// 画图
		async draw(canvas, ctx, width, height) {
			// 清空画布
		...
		drawText(canvas, ctx, item, width, height) {
			...
			item.x === 'center' ? (item.x = (width - ctx.measureText(item.content).width) * 0.5) : ''
