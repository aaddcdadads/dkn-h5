<template>
  <u-row type="flex" :align="align" :justify="justify" class="hm-grid" ref="hmGrid">
    <u-col v-for="col in cols" :key="col.key" :span="col.span">
      <view class="ant-col-slot">
        <slot :name="`ant-col-slot-${col.key}`">
          <view class="hm-slot" :data-slot-name="`ant-col-slot-${col.key}`"><view class="sortable-list"></view></view>
        </slot>
      </view>
    </u-col>
  </u-row>
