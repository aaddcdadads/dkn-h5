...
export default {
  ...
  if (!data) {
    ...
    uni.redirectTo({
      url: "/pages/haomo/1753965929131151361/page?activityId=" + this.activityId + "&channelId=" + this.channelId + "&orderId=" + this.orderId
    })
  } else {
    const queryString = data.split('?')[1]; // 获取？后面的查询参数部分
    const storeId = queryString && queryString.indexOf("=") ? queryString.split('=')[1] : "-1";
    ...
    this.$getAction("/api/dkn/store/queryById", {
      ...
      uni.redirectTo({
        url: "/pages/haomo/1754031921416900610/page?activityId=" + this.activityId + "&storeId=" + storeId + "&channelId=" + this.channelId + "&orderId=" + this.orderId
      })
