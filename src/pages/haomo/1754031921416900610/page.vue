<template>
  <view class="page card">
    <view class="page-wrapper">
      ...
      <hm-uview-field ref="roundField" v-model:value="roundField.value" label="当前核销" placeholder="当前核销轮次" label-width="160" :label-align="'center'" maxlength="-1" icon="" right-icon="" :disabled="true" class="ele-roundField"> 
    </hm-uview-field>
    </view> 
            <view class="ele-wrapper ele-wrapper-timeField">
      <hm-uview-field ref="timeField" v-model:value="timeField.value" label="核销时间" placeholder="当前核销轮次核销时间范围" label-width="160" :label-align="'center'" maxlength="-1" icon="" right-icon="" :disabled="true" class="ele-timeField"> 
    </hm-uview-field>
    </view> 
...
export default {
  ...
  this.getCurrentRound = function(roundDates, currentTime) {
    ...
    onMounted() {
  console.log("获取数据---", this.activityId, this.storeId,this.orderId);
  //查询订单信息
  let params = {}
  if (this.orderId) {
    params = {
  ...
  this.$getAction("/api/dkn/viewRegistrationOrders/list", params).then(res => {
    ...
    console.log("resqewqw--", this.currentRound, roundDates[this.currentRound])
  });
  //查询扫码的门店信息
