<template>
	<view class="u-switch" :class="[getValue() == true ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]">
      ...
      modelValue: {
		...
		computed: {
			switchStyle() {
				...
				style.backgroundColor = this.getValue() ? this.activeColor : this.inactiveColor;
				return style;
			},
			loadingColor() {
				return this.getValue() ? this.activeColor : null;
			}
		},
		methods: {
      getValue(){
        ...
        // #endif
          ...
          this.$emit("update:modelValue", !this.getValue());
					...
					this.$nextTick(() => {
						this.$emit('change', this.getValue() ? this.activeValue : this.inactiveValue);
					})
				}
			}
		}
