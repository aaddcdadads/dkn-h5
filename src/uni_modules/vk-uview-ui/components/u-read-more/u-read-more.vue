...
export default {
	...
	mounted() {
		this.cAllowClick = this.allowClick;
		this.$nextTick(() => {
			this.init();
		})
	},
	methods: {
		init(val) {
			this.$uGetRect('.' + this.elId).then(res => {
				...
				if (res.height > uni.upx2px(this.showHeight) || val > uni.upx2px(this.showHeight)) {
